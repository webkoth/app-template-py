...
```makefile
# Единая точка входа. Прогоняется перед любым коммитом.
# Табуляции в начале строк рецептов обязательны — это Make.

.PHONY: check check-backend check-frontend dev migrate install
...
check-openapi:
	...
	diff -q frontend/src/api/schema.d.ts frontend/src/api/schema.d.ts.new \
		|| (echo "Типы клиента разошлись со схемой. Выполни: make openapi"; exit 1)
...
- [ ] **Шаг 2: Проверить, что бэкенд-проверки проходят**

Run: `make check-backend`
Expected: ruff, mypy, lint-imports и pytest зелёные (тестов пока три файла).
...
Внимание: в `format_money` пробелы неразрывные (` `). Тест сравнивает с
обычным пробелом в исходнике — привести тест к тому же виду, заменив пробелы
в ожидаемых строках на ` `, либо сравнивать через
`.replace(" ", " ")`. Выбрать второе: читаемость теста важнее.
...
class TestFormat:
    ...
    def fmt(minor: int) -> str:
        ...
        return format_money(minor).replace(" ", " ")
...
- **Сгенерированную миграцию читают глазами перед применением.**
  ...
  смену типа и никогда — смену смысла.
