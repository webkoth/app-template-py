...
# Состав схемы — в app/models.py, тем же списком, что и у миграций. Схема
# тестовой базы создаётся из метаданных, а туда таблица попадает, только
# если её модуль кто-то импортировал: без этой строки состав схемы зависел бы
# от того, какие роутеры уже подключены к приложению, то есть менялся бы от
# задачи к задаче. Первый же тест фичи упал бы с «relation does not exist», и
# причину искали бы в тесте, а не в цепочке импортов.
# Форма импорта здесь существенна: `import app.models` связала бы имя
# `app` с ПАКЕТОМ и затёрла бы приложение FastAPI, взятое строкой выше, —
# `app.dependency_overrides` в фикстуре клиента упало бы с AttributeError у
# всех тестов разом. Проверено.
...
async def session() -> AsyncIterator[AsyncSession]:
    ...
    ЧЕГО ЭТА ФИКСТУРА НЕ ВИДИТ. Откат здесь вложенный, и объекты он гасит
    выборочно — только изменённые. Настоящий `rollback()` гасит ВСЕ объекты
    сессии, и следующее же чтение любого поля в async-коде падает
    MissingGreenlet: устаревший атрибут тянет догрузку синхронно.

    Это не теория. Воркер после отказа обработчика делал `rollback()` и
    сразу звал `jobs.fail`, а тот первой строкой читает `job.attempts`. На
    контуре каждый отказ обработчика оставлял бы задачу «выполняющейся» без
    причины в поле `error` — ровно тот молчаливый отказ, против которого
    очередь и написана. Все тесты на этой фикстуре были зелёными.

    Поэтому: код, который откатывает транзакцию и продолжает работать с теми
    же объектами, проверяй на `new_session` — там сессия настоящая.
    """
