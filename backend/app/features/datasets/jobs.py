...
async def parse(session: AsyncSession, job: Job) -> dict[str, Any]:
    ...
    #
    # Одной транзакцией это НЕ идёт, вопреки тому, что здесь стояло раньше:
    # `jobs.heartbeat` ниже делает commit, и первый её вызов приходится на
    # нулевую строку — то есть удаление коммитится вместе с первой строкой, а
    # дальше данные едут пачками отдельными коммитами. Уникальности
    # (dataset_id, index) это всё равно не нарушает, но по другой причине:
    # удаление проходит РАНЬШЕ любой вставки, а не вместе с ней.
    #
    # Следствие, которое из-за неверного комментария не замечали: повторный
    # разбор, окончательно упавший после удаления, оставлял бы у таблицы
    # старую сводку при усечённых строках — экран говорил бы «разобрана, N
    # строк» над наполовину пустой таблицей. Поэтому сводка гасится здесь же,
    # и всё время повторного прохода состояние честное: «считается».
