...
if (env !== null) {
  ...
  // Каталог загрузок. Строки в `.env` может не быть вовсе — в образце она
  // закомментирована, и это рабочее состояние: путь тогда считается от
  // самого `.env`, то есть локально `uploads/` в корне репозитория, а на
  // контуре `/var/www/<слаг>/uploads`. Проверяется поэтому не наличие
  // строки, а то, что заданное значение годится.
  //
  // Годится только абсолютный путь. Относительный (и пустая строка — это
  // `Path(".")`) роняет приложение на старте: рабочий каталог у
  // веб-процесса, воркера очереди и команд `make` разный, и файл,
  // загруженный минуту назад, оказался бы «не найден». Приложение это
  // отвергает само, но узнаётся об этом при первом запуске после правки
  // `.env` — здесь же это одна строка и до запуска.
  const uploadDir = value("UPLOAD_DIR")
  check(
    "UPLOAD_DIR задан абсолютным или пропущен",
    !/^UPLOAD_DIR=/m.test(env) || /^([/~]|[A-Za-z]:[\\/])/.test(uploadDir),
    uploadDir === ""
      ? "пустое значение — это путь `.`; убери строку целиком, тогда каталог возьмётся рядом с .env"
      : "путь обязан быть абсолютным: у приложения, воркера и make рабочие каталоги разные"
  )
}
